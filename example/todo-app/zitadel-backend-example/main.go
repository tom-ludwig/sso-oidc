...
var (
	...
	key    = flag.String("key", "./key.json", "path to your key.json")
...
func main() {
	...
	router.Handle("/api/add-task", mw.RequireAuthorization(authorization.WithRole(`admin`))(http.HandlerFunc(
		func(w http.ResponseWriter, r *http.Request) {
			...
			slog.Info("admin added task", "id", authorization.UserID(r.Context()), "task", req.Task)

			// inform the admin about the successful addition
			err = jsonResponse(w, fmt.Sprintf("task `%s` added", req.Task), http.StatusOK)
			if err != nil {
	...
	c := cors.New(cors.Options{
		AllowedOrigins:   []string{"http://foo.com", "http://localhost:5173"},
